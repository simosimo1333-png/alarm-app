...
ネットワーク・APIキー不要のオフライン単体テストを同梱しています。
...
指標計算・シグナル判定・リスク管理（損切り/利確・日次損失・連続エントリー禁止・
証拠金維持率）・足生成・HMAC署名・建玉同期を検証します。

---

## 実取引（本番）への移行手順

本番では DRY_RUN にない以下の安全機構が自動で働きます。

- **本番プリフライト**: 起動時に Private API（資産・有効注文）へ1回アクセスし、
  認証・接続と証拠金維持率を検証してから取引を開始します（失敗時は起動せず停止）。
- **既存建玉の引き継ぎ**: 起動時に対象通貨ペアの建玉があれば、ボットが SL/TP 管理下に
  置きます（多重エントリーを防止）。
- **約定後の建玉同期**: 成行約定の後に `openPositions` を照会し、実際の
  `positionId`・約定価格を取り込みます。確認できない場合は安全停止し、手動確認を促します。

### 移行ステップ

1. `DRY_RUN=true` のまま数日〜数週間運用し、ログでシグナル品質と損益感覚を確認する。
2. **`broker_gmo_fx.py` のエンドポイント／パラメータ／レスポンス項目名を
   [公式API仕様](https://api.coin.z.com/fxdocs/) と必ず照合する**
   （本リポジトリは GMO の API 体系に基づく実装ですが、最終確認は必須）。
3. GMOコインで API キー（取引権限あり）を発行し、`GMO_API_KEY` / `GMO_API_SECRET` を設定。
4. **最小ロット**（`ORDER_SIZE` を最小単位）かつ `MAX_DAILY_LOSS_JPY` を小さく設定する。
5. `NOTIFY_CHANNEL` を `line` か `email` にして通知が届くことを確認する。
6. 経済指標カレンダーを `NEWS_WINDOWS` に反映する。
7. 取引時間内・少額・短時間で `DRY_RUN=false` を試し、約定・建玉同期・決済・通知が
   想定どおり動くことを実弾で確認してから本格運用へ。

### 移行前チェックリスト

- [ ] `DRY_RUN=true` で十分な期間ログを確認した
- [ ] `broker_gmo_fx.py` を公式仕様と照合した
- [ ] `python -m unittest discover -s tests` が全て通る
- [ ] API キーは取引権限つき、出金権限なしで発行した
- [ ] `ORDER_SIZE` を最小単位、`MAX_DAILY_LOSS_JPY` を少額に設定した
- [ ] 通知（LINE/メール）が届くことを確認した
- [ ] 経済指標カレンダーを `NEWS_WINDOWS` に反映した
- [ ] 少額・短時間の実弾テストで約定・決済・建玉同期を確認した

> ⚠️ 本コードはあなた自身の GMO 口座・資金で動かす個人ツールです。`DRY_RUN=false` への
> 切り替えと実発注は、上記を満たしたうえで**ご自身の判断と責任**で行ってください。
